...
export function ProjectCard({
  ...
  return (
    <Link href={slug ? `/projects/${slug}` : href || "#"} className="block h-full">
      <Card
        className={cn(
              ...
              {links?.map((linkItem, idx) => (
                <div
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    window.open(linkItem.href, '_blank');
                  }}
                  className="cursor-pointer"
                >
                  <Badge className="flex gap-2 px-2 py-1 text-[10px] hover:bg-primary/90">
